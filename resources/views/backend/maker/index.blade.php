				...
				<div class="row">
					...
					<div class="col-xs-6">
						<div class="dataTables_filter">
							...
							<label> </label>
							<label>
								<a class="btn" href="{{ route('backend.maker.create') }}">Thêm mới</a>
							</label>
						</div>
					</div>
				</div>
				<table id="listProduct" class="table table-hover">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-right">
								<a class="edit btn btn-sm btn-default" href="{{ route('backend.maker.edit', $maker['id']) }}"><i class="icon-note"></i></a>
								<a class="delete btn btn-sm btn-danger" href="{{ route('backend.maker.destroy', $maker['id']) }}"><i class="icons-office-52"></i></a>
                        	</td>
						</tr>
						@endforeach
					</tbody>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
							Showing {{ $maker_paginate->count() }} to {{ $maker_paginate->currentPage() }} of {{ $maker_paginate->total() }} entries
						</div>
					</div>
					<div class="col-md-6">
						<div class="dataTables_paginate paging_simple_numbers">
							{{ $maker_paginate->appends(Request::all())->links() }}
						</div>
					</div>
				</div>
			</div>
